...
namespace RestService
{
    ...
    public class RestServiceImpl : IRestServiceImpl
    {
        ...
        private string GetMobileNumber(string Aadhaar)
        {
            ...
            SqlDataAdapter adp = new SqlDataAdapter("Select top 1 Mobile_Number from AWWMaster where REPLACE(AadhaarNo,'-','')='" + Aadhaar + "'", ConfigurationManager.ConnectionStrings["AWWs"].ConnectionString);
        ...
        private Boolean SaveData(string OTP,string Mobile)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("insert into tbl_OTP(MobileNo,OTP,IsActive,Datetime) values(@Mobile,@OTP,@A,@Date)", con);
        ...
        public string Save_AWW_Details(AWW_User_Details aww_user_details)
        {
            ...
            SqlCommand cmd = new SqlCommand("insert into DATA(Aanganwadi_Name,PhoneNumber,TotalEnrollments,Issues_Feedbacks,EntryDate,IMEINo) values(@Aanganwadi_Name,@PhoneNumber,@TotalEnrollments,@Issues_Feedbacks,@EntryDate,@IMEINo)", con);
        ...
        private string CheckLogIn(string Aadhaar,string OTP)
        {
            ...
            try {
                DataTable dt = new DataTable();
                SqlDataAdapter adp = new SqlDataAdapter("Select *  from tbl_OTP O inner join AWWMaster  A on A.Mobile_Number=O.MobileNo  where REPLACE(A.AadhaarNo,'-','')='" + Aadhaar + "' and O.OTP='" + OTP + "'", ConfigurationManager.ConnectionStrings["AWWs"].ConnectionString);
